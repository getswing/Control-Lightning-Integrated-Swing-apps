...
- Melindungi dashboard dan API dengan login berbasis cookie session.
...
- Menjalankan scheduler otomatis setiap 5 detik.
...
| `APP_PASSWORD` | Password untuk login web. |
| `APP_SESSION_SECRET` | Secret untuk tanda tangan cookie session. |
...
Untuk Bardi 2 gang yang dipakai sebagai 2 lapang, satu device ID bisa dibuat menjadi dua logical lamp:
...
Booking bentrok dihitung berdasarkan `deviceId + code`, jadi Lapang 1 dan Lapang 2 bisa booking di jam yang sama.
...
Endpoint kontrol lampu, jadwal, dan booking wajib login. Jika belum login, backend mengembalikan HTTP `401`.
...
Login memakai password dari `APP_PASSWORD`.
...
Jika berhasil, backend mengirim cookie `tuya_light_session`.
...
```js
setInterval(runAutomation, 5_000);
```

Setiap 5 detik server mengecek waktu saat ini. Transisi jadwal harian dan start/end booking hanya diproses sekali per menit, tetapi pengecekan booking aktif berjalan setiap 5 detik.
...
- Selama booking aktif, server mengambil status device dari Tuya Cloud setiap 5 detik.
...
- Ganti `APP_PASSWORD` sebelum publish.
- Pakai `APP_SESSION_SECRET` panjang dan acak di server production.
